...
namespace RoomMoonLune
{
    class SmokeParticles : Sprite
    {
        ...
        public SmokeParticles(Texture2D texture, int width, int height, Random rand):base(texture,width,height)
        {
            isAlive = false;
            aliveTime = 3.0f;
            curTime = 0.0f;
            ...
            if (isAlive)
            {
                ...
                if (curTime < aliveTime)
                {
                    if (scale.X < 2.4f)
                    {
                        ...
                        {
                            color = Color.Lerp(color, Color.White, 3 * (float)gameTime.ElapsedGameTime.TotalSeconds);
                        }
                    }
